...
func (this *Encrypter) EncryptFile(key []byte, filename string, outFilename string) (string, error) {
...
func (this *Encrypter) DecryptFile(key []byte, filename string, outFilename string) (string, error) {
    if len(outFilename) == 0 {
...
func _Load() {
    ...
    if *encFlag {
        outFilename, err := encrypter.EncryptFile(key, filename, "")
        if err != nil {
            handle.Error(err)
        }
        fmt.Println("Encrypted output file:", outFilename)
    } else if *decFlag {
        outFilename, err := encrypter.DecryptFile(key, filename, "")
        if err != nil {
            handle.Error(err)
        }
